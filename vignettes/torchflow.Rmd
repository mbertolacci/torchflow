...
`input_size = 1`. In this case, affine coupling blocks use a single affine
coupling internally. Permutation layers are no-ops, and repeated affine blocks
compose to a single affine transform, so these constructors warn when used with
